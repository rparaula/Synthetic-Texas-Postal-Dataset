...
DROP PROCEDURE IF EXISTS `pkg_init_determine_smartlocker_destination_facility`;
DELIMITER $$

CREATE PROCEDURE `pkg_init_determine_smartlocker_destination_facility`(
...
WHERE `ROUTINE_SCHEMA` = 'postal_bi_system'
  AND `ROUTINE_NAME` = 'pkg_init_determine_smartlocker_destination_facility';
